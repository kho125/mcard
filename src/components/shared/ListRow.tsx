...
function ListRow({ as = 'li', left, contents, right, withArrow, onClick }: ListRowProps) {
  return (
    <Flex as={as} css={listRowContainerStyles} onClick={onClick} align='center'>
      <Flex css={listRowLeftStyles}>{left}</Flex>
      <Flex css={listRowLContentsStyles}>{contents}</Flex>
      <Flex>{right}</Flex>
...
const listRowLContentsStyles = css`
  flex: 1;
`
...
function ListRowSkeleton() {
  return (
    <Flex as='li' css={listRowContainerStyles} align='center'>
      ...
      <Flex css={listRowLContentsStyles}>
        <ListRow.Texts
          title={
            <>
